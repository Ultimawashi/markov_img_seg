import numpy as np
from utils import generate_semipos_sym_mat, np_multivariate_normal_pdf, np_multivariate_normal_pdf_marginal, algorithm_u
from scipy.stats import multivariate_normal

# A TMC is equivalent to a PMC in which the hidden variable are multidimensional, the only difference is that we don't
# want to predict all the component of the hidden vector but just one component (so a marginalisation step will be
# necessary, which is not the case always the case in multidimensional PMC)
...
class TMC_ctod:
    ...
    def give_param(self, c, u, mu, sigma):
        nb_class = self.nbc_x * self.nbc_u
        self.p = np.outer(np.sum(c, axis=1),np.sum(u,axis=1)).flatten()
        t = (c.T / np.sum(c, axis=1)).T
        b = np.repeat(t, self.nbc_u, axis=0)
        ut = (u.T / np.sum(u, axis=1)).T
        ut = [[ut for n1 in range(self.nbc_x)] for n2 in range(int(nb_class / self.nbc_u))]
        ut = np.block(ut)
    ...
    def init_kmeans(self, data, labels):
        ...
        b = np.repeat(a, self.nbc_u, axis=0)
        ...
        M = sum([np.array(
            [[(hidden[i - 1] == k and hidden[i] == l) * data[i - 1:i + 1].flatten() for l in
              range(self.nbc_x)] for k in
             range(self.nbc_x)]) for i in range(1, len(data))]) / sum([np.array(
            [[np.full((2 * len(data[0])), (hidden[i - 1] == k and hidden[i] == l)) for l in range(self.nbc_x)]
             for k in
             range(self.nbc_x)]) for i in range(1, len(data))])

        Sig = sum([np.array(
            [[((hidden[i - 1]) == k and (hidden[i]) == l) * np.outer(
                data[i - 1:i + 1].flatten() - M[k, l],
                data[i - 1:i + 1].flatten() - M[k, l]) for l
              in
              range(self.nbc_x)] for k in
             range(self.nbc_x)]) for i in range(1, len(data))]) / sum([np.array(
            [[np.full((2 * len(data[0]), 2 * len(data[0])),
                      ((hidden[i - 1]) == k and (hidden[i]) == l)) for
              l in range(self.nbc_x)] for k in
             range(self.nbc_x)]) for i in range(1, len(data))])
        ...
        for l in range(nb_class):
            for k in range(nb_class):
                if count % 2 == 0:
                    self.mu[l][k] = M[(l//self.nbc_u)][(k//self.nbc_u)] - ((count / (nb_class ** 2 / 2)) * np.sum(Sig[(l//self.nbc_u)][(k//self.nbc_u)] / 2, axis=1))
                else:
                    self.mu[l][k] = M[(l//self.nbc_u)][(k//self.nbc_u)] + ((count / (nb_class ** 2 / 2)) * np.sum(Sig[(l//self.nbc_u)][(k//self.nbc_u)] / 2, axis=1))
                self.sigma[l][k] = Sig[(l//self.nbc_u)][(k//self.nbc_u)]
                count = count + 1
    ...
    def seg_mpm(self, data, all_possible_seg=False):
        forward, backward, _ = self.get_forward_backward(data)
        res = [None] * len(data)
        if all_possible_seg:
            idxs = algorithm_u(range(self.nbc_x*self.nbc_u), self.nbc_x)
            idxs = [a for a in idxs if all(len(item) == int(self.nbc_u) for item in a)]
        for i in range(0, len(data)):
            if all_possible_seg:
                aux = (forward[i] * backward[i])
                aux = aux / np.sum(aux)
                aux = [np.array([np.sum(aux[ids]) for ids in idx]) for idx in idxs]
                res[i] = [a.tolist().index(max(a)) for a in aux]
            else:
                aux = (forward[i] * backward[i])
                aux = aux / np.sum(aux)
    ...
    def simul_hidden_apost(self, data, backward, pcond):
        res = [None] * len(data)
        aux = np.sum(
            np.multiply((self.t.T*self.p).T, np_multivariate_normal_pdf_marginal(data[0], self.mu, self.sigma, (len(data[0]) - 1))),
            axis=1) * backward[0]
        test = np.random.multinomial(1, aux / np.sum(aux))
        res[0] = test.tolist().index(1)
    ...
    def generate_sample(self, length, dimY):
        ...
        for i in range(2, length):
            auxpb = np.multiply((self.t.T*self.p).T, np_multivariate_normal_pdf_marginal(visible[i - 1], self.mu, self.sigma,
                                                                            (dimY - 1)))
    ...
    def get_forward_backward(self, data):
        ...
        for l in range(1, len(data)):
            ...
            if pcond[l - 1] is None:
                ...
                auxpb2 = np.sum(np.multiply((self.t.T*self.p).T,
                                            np_multivariate_normal_pdf_marginal(data[l - 1], self.mu, self.sigma,
                                                                                (len(data[l - 1]) - 1))), axis=1)
                pcond[l - 1] = (auxpb.T / auxpb2).T

            if pcond[k] is None:
                auxpb = np.multiply((self.t.T*self.p).T, np_multivariate_normal_pdf(data[k:k + 2].flatten(), self.mu, self.sigma))
                auxpb2 = np.sum(np.multiply((self.t.T*self.p).T, np_multivariate_normal_pdf_marginal(data[k], self.mu, self.sigma,
                                                                                        (len(data[k]) - 1))), axis=1)
